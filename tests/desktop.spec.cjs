...
test('demo, figures, project roundtrip, and PNG export',async()=>{
 ...
 await page.screenshot({path:'test-results/figure-studio.png'});
...
test('dense graphs keep compact export dimensions and diagonal labels',async()=>{
 ...
 const initialWidth=await page.locator('#chart').evaluate(canvas=>canvas.width);
 ...
 for(const type of ['grouped','bar']){
  await page.evaluate(()=>{window.labelAngles=[];const proto=CanvasRenderingContext2D.prototype;if(!window.originalRotate){window.originalRotate=proto.rotate;proto.rotate=function(angle){window.labelAngles.push(angle);return window.originalRotate.call(this,angle);};}});
  await page.selectOption('#graphType',type);
  const dimensions=await page.locator('#chart').evaluate(canvas=>({width:canvas.width,cssWidth:parseFloat(canvas.style.width),available:canvas.parentElement.clientWidth,angles:window.labelAngles}));
  expect(dimensions.width).toBe(initialWidth);expect(dimensions.cssWidth).toBeLessThanOrEqual(820);expect(dimensions.cssWidth).toBeLessThanOrEqual(dimensions.available);
  expect(dimensions.angles.filter(a=>Math.abs(a+Math.PI/4)<.001).length).toBe(type==='grouped'?12:48);
